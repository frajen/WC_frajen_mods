...
def parse(parser):
    ...
    ### shop mods
    shops.add_argument("-shopsnogood", "--shops-have-no-good-items", action = "store_true",
                       help = "Good tier items are not sold in shops")
    shops.add_argument("-maxshopcount", "--max-shop-count", default = 8, type = int,
                              choices = range(1, 9), metavar = "COUNT", 
                              help = "Shops can sell maximum %(metavar) items")
...
def flags(args):
    ...
    ### shop mod
    if args.shops_have_no_good_items:
        flags += " -shopsnogood"
    ### toss out any numbers over 8 (vanilla)
    if args.max_shop_count < 8:
        flags += f" -maxshopcount {args.max_shop_count}"
...
def options(args):
    ...
    result.extend([
        ...
        ("No good items", args.shops_have_no_good_items),
        ("Max shop count", args.max_shop_count),
    ])
    return result
