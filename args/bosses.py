...
def parse(parser):
    ...
    ### enemy mods
    bosses.add_argument("-spdbst", "--fast-enemies", action = "store_true",
                        help = "All enemies have 170-230 speed")
    bosses.add_argument("-ezbuff", "--easy-boss-buff", action = "store_true",
                        help = "'Easier' bosses get buffs")
    bosses.add_argument("-bemod", "--boss-experience-mod", default = None, type = int,
                        metavar = "PERCENT", choices = range(101),
                        help = "Percent modifier to boss XP")
...
def process(args):
    ...
    if args.boss_experience_mod is not None:
        args.boss_experience_percent = args.boss_experience_mod
        args.boss_experience_mod = True
...
def flags(args):
    ...
    ### enemy mods
    if args.fast_enemies:
        flags += " -spdbst"
    if args.easy_boss_buff:
        flags += " -ezbuff"
    if args.boss_experience_mod:
        flags += f" -bemod {args.boss_experience_percent}"
...
def options(args):
    ...
    if args.boss_experience_mod:
        result.append(("Boss XP %", args.boss_experience_percent))               

    result.append(("No Undead", args.boss_no_undead))
    result.append(("Marshal Keep Lobos", args.boss_marshal_keep_lobos))
    result.append(("Fast Enemies", args.fast_enemies))
    result.append(("Easy Boss Buff", args.easy_boss_buff))
