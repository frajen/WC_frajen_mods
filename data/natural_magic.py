...
class NaturalMagic:
    ...
    ### Updated for extended command randomization and tiered natural magic
    def mod_learners(self):
        import random        
        from data.characters import Characters
        possible_learners = list(range(Characters.CHARACTER_COUNT - 2)) # exclude gogo/umaro

        ### if Magic commands are not universal to characters, only allow
        ### characters with Magic/X Magic to have natural magic
        ### If using command tiering, putting natural magic on charaters
        ### with Magic/X Magic gets messed up, since it needs to be done AFTER event 
        ### determination, which is after mod_learners is typically called.
        ### event_check and after_battle_check_mod reserves Space. 
        ### WC seems to hard code natural magic users as Terra/Celes if you don't
        ### run mod_learners in typical WC position
        if self.args.magic_swap_chance > 0:
            magic_users = self.characters.get_characters_with_command("Magic")
            xmagic_users = self.characters.get_characters_with_command("X Magic")
            possible_magic_learners = [x for x in magic_users if x <= 11]    # only pick main characters
            possible_xmagic_learners = [x for x in xmagic_users if x <= 11]  # only pick main characters
            possible_learners = list(set(possible_magic_learners + possible_xmagic_learners))
            ### if no one can use Magic or X Magic, just set natural magic randomly
            if len(possible_learners) == 0:
                possible_learners = list(range(Characters.CHARACTER_COUNT - 2)) # exclude gogo/umaro
            #print(possible_learners)
        ...
        if self.args.natural_magic1 == "random":
            ...
            self.characters.get(self.learner1).natural_magic_user = 1
            #for character in self.characters:
                #if character.id == self.learner1:
                    #character.natural_magic_user = 1
        ...
        if self.args.natural_magic2 == "random":
            ...
            self.characters.get(self.learner2).natural_magic_user = 1
    ...
    def randomize_spells1(self):
        ### Mod for random tiered natural magic
        if self.args.random_tiered_natural_spells1:
            random_spells = self.spells.get_random_tiered(count = len(self.terra_spells), exclude = self.args.remove_learnable_spell_ids)
        else:
    ...
    def randomize_spells2(self):
        ### Mod for random tiered natural magic
        if self.args.random_tiered_natural_spells2:
            random_spells = self.spells.get_random_tiered(count = len(self.celes_spells), exclude = self.args.remove_learnable_spell_ids)
        else:
