...
class Maps():
    ...
    ### Mod to make certain maps warpable
    def mod_warpable_maps(self):
        #20-22, 24-30, 43, 48-52 Narshe WoB | #114 Lete | #125-126, 317, 319-320 Cyan's Dream | #272 Mtek Cart Ride Room
        #new_warpables = [20, 21, 22, 24, 25, 26, 27, 28, 29, 30, 43, 48, 49, 50, 51, 52, 114, 125, 126, 272, 317, 319, 320]
        # #351 burning house
        new_warpables = [20, 21, 22, 24, 25, 26, 27, 28, 29, 30, 43, 48, 49, 50, 51, 52, 114, 125, 126, 272, 317, 319, 320, 351]
        
        # loop through all maps
        for map_index, cur_map in enumerate(self.maps):
            if map_index in new_warpables:
    ...
    def write(self):
        print_debug = False
        import random  #used by randomized map palette colors
        self.npcs.write()
        self.chests.write()
        self.events.write()
        self.exits.write()
        ...
        # Mod to make certain maps warpable
        if self.args.more_warp:
            self.mod_warpable_maps()

        for map_index, cur_map in enumerate(self.maps):
            # debug: print all map events
            if print_debug and map_index < 415:
                first_event_id = (self.maps[map_index]["events_ptr"] - self.maps[0]["events_ptr"]) // MapEvent.DATA_SIZE
                event_count = self.get_event_count(map_index)
                
                for offset in range(event_count):
                    #for event in self.events.events:
                    event_metadata = self.events.events[first_event_id + offset].get_meta()
                    print(str(map_index) + " | " + str(eventID) + " | " + event_metadata)
                    eventID += 1

            ### Experimental: randomize map palette colors
            if 0 == 1:
                self.properties[map_index].paletteindex = random.choice([10, 36, 5, 23, 24, 6, 0, 9, 21, 34, 15, 37, 22, 1, 31, 32, 29, 20, 18, 3])
                self.properties[map_index].paletteanimationindex = random.randint(0,8)

            ### Disable random encounters on non-world maps
            #self.properties[map_index].enable_random_encounters = 0
