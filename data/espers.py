...
class Espers():
    ...
    def __init__(self, rom, args, spells, characters):
        ...
        ### Mod for tiered Espers
        self.esper_rank = {}
        self.tiered_order = []
        self.tiering_has_run = False
...
##    ### for CG location tiering
    def get_random_tiered_esper(self, event_depth, event):
        if not self.available_espers:
            return None
        #rand_esper = random.sample(self.available_espers, 1)[0]
        ...
        for x, depth in event_depth.items():
            if depth > max_depth:
                max_depth = depth
        ### split Esper ranks into number of depth values
        #print("Max depth: " + str(max_depth))
        max_espers = 27          ### every Esper has to be eventually be placed
        tier_size = int(max_espers / max_depth)  # round down
        #print(tier_size)
        ### optional: max_espers/max_depth could be adjusted to match a total Espers required value.
        ### larger tier_sizes = better Espers more quickly
        ### realistically never see tier_sizes of more than 4?
        tier_variation_odds = {
            1 : [100],
            2 : [75, 100],
            3 : [65, 80, 100],
            4 : [55, 80, 95, 100],
            5 : [55, 80, 95, 98, 100],
            6 : [55, 80, 95, 98, 99, 100],
            7 : [55, 80, 94, 97, 98, 99, 100],
            8 : [55, 80, 93, 96, 97, 98, 99, 100],
            9 : [55, 80, 92, 95, 96, 97, 98, 99, 100],
            }
        odds = tier_variation_odds[tier_size]
            
        if not self.tiering_has_run:
            # use BC's spell rank for now
            spell_rank = {
                0 : 19,   # Fire
                1 : 19,   # Ice
                2 : 18,   # Bolt
            ...
            for esper in self.espers:
                #print(str(esper.name) +": "+ str(esper.id))
                spellranksum = 0
                spellstr = ""
                for spell in esper.spells:
                    if spell.id != 255:
                        spellstr += id_name[spell.id] + ", "
                        spellranksum += int(spell_rank[spell.id])
                    #print(spell.id)
                    #print(spell_rank[spell.id])
                self.esper_rank[esper.name] = spellranksum
                #print(str(esper.name) +": "+str(esper_rank[esper.name]) + " : " +
                      #spellstr)

            print("Max depth: " + str(max_depth))
            
            temprank = {k: v for k, v in sorted(self.esper_rank.items(), key=lambda item: item[1])}
            esperrank = list(temprank.items())
            print(esperrank)
            #input("X")

            ### get Esper rank ordering
            pickorder = []
            import random
            while len(pickorder) < 27:
                temptier = []

                ### handle the last 
                if len(esperrank) < tier_size:
                    odds = tier_variation_odds[len(esperrank)]
                    for x in range(len(esperrank)):
                        temptier.append(esperrank[x])
                else:
                    for x in range(tier_size):
                        temptier.append(esperrank[x])

                #print(temptier)
                ### do for the last
                #if len(temptier) == 1:
                    #pickorder.append(temptier[0][0])
                    #esperrank.remove((temptier[0][0], temptier[0][1]))
                    #exit
                #else:
                ...
                while(roll > odds[x]):
                    x = x + 1
                #print(temptier[x][0])
                pickorder.append(temptier[x][0])
                esperrank.remove((temptier[x][0], temptier[x][1]))


            print(pickorder)
            #input("Z")
            self.tiered_order = pickorder
            self.tiering_has_run = True
        ### end of first time run only

        ### get the individual event depth to determine where in the
        ### Esper list to pull from
        eventdepth = event_depth[event.name()]
        max_espers = 27
        #esperindex = eventdepth * tier_size   # changed to allow eventdepth to translate to index 0
        esperindex = (eventdepth-1) * tier_size
        #print(esperindex)
        ### pull from the top of the list if the event depth is higher
        ### than what already exists
        if esperindex >= len(self.tiered_order):
            esperindex = len(self.tiered_order)-1
        try:
            choice = self.tiered_order[esperindex]
        except IndexError:
            print(esperindex)
            print(self.tiered_order)
        self.tiered_order.remove(choice)
        #print(choice)
        print(str(event.name()) + ": " + str(eventdepth) +
              " / Esper: " + choice + " | " + str(self.esper_rank[choice]))

        rand_esper = self.esper_names.index(choice)
        self.available_espers.remove(rand_esper)
        return rand_esper
    ### end of section for CG location tiering
