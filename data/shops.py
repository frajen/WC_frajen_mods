...
class Shops():
    ...
    def remove_excluded_items(self):
        ...
        ### Mod to not sell any reward-tier items
        if self.args.shops_have_no_good_items:
            from constants.items import good_items
            reward_tier_items = [name for name in good_items]
            if self.args.stronger_atma_weapon:
                reward_tier_items.append("Atma Weapon")
            for reward_name in reward_tier_items:
                exclude.append(self.items.get_id(reward_name))

            #add this one if modifying the high tier item pool
            # used for BC-WC seed 3 when items were the only things that taught magic
            #for item in self.items.GOOD:
                #exclude.append(item)
        ### End of: Mod to not sell any reward-tier items
        ...
        # these can be incorporated into the above loop
        ### Mod to limit item count per shop to item_max + 1: -maxshopcount <number>
        if self.args.max_shop_count < 8:
            print_debug = False
            item_max = self.args.max_shop_count - 1
            for shop in self.shops:
                item_counter = 0
                newlist = []
                if print_debug:
                ...
                for item in shop.items:
                    if (item_counter <= item_max) or (item == 255):
                        newlist.append(item)
                    item_counter = item_counter + 1
                #done with loop through each item in a shop
                while len(newlist) < 8:
                    newlist.append(255)
                shop.items = newlist
                shop.item_count = min(item_counter, item_max)
                if print_debug:
                    print(shop.items)
    ...
    def mod(self):
        ...
        # Mod: swap this to prevent Dried Meats from getting chopped off
        self.remove_excluded_items()
        self.assign_dried_meats()
