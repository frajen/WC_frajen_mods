...
class Rages():
    ...
    ### Mod for tiering start rages
    def start_tiered_random_rages(self):
        print_debug = False
        import random
        self.init_data.clear_all()

        from data.rage_tiers import rage_tiers
        self.rage_tiers = rage_tiers

        def get_rage(tiers):
            from data.rage_tiers import weights
        ...
        # get initial number of rages
        number_initial_rages = random.randint(self.args.start_rages_random_min, self.args.start_rages_random_max)
        from data.enemy_names import enemy_id_name
        initial_rages = []
        for x in range(number_initial_rages):
            rage_id = get_rage(self.rage_tiers)
            if print_debug:
                print(rage_id)
                
            while(rage_id in initial_rages):
                rage_id = get_rage(self.rage_tiers)
            initial_rages.append(rage_id)
            if print_debug:
                print(initial_rages)
                print(enemy_id_name[rage_id])

        # validate rage list is unique
        if len(initial_rages) != len(set(initial_rages)):
            if print_debug:
                print(initial_rages)
        else:
            # for debugging rage names only
            if print_debug:
                initial_rage_names = []
                for rage_id in initial_rages:
                    initial_rage_names.append(enemy_id_name[rage_id])
                print(len(initial_rages))
                print(initial_rage_names)
    ...
    def mod(self):
        # add option for tiered starting rages
        if self.args.start_rages_random:
            if self.args.tiered_rages:
                self.start_tiered_random_rages()
            else:
