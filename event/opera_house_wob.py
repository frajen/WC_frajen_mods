...
class OperaHouseWOB(Event):
    ...
    def ultros_battle_mod(self):
        ...
        ### Mod to force Grand Finale to play during this battle
        # space = Reserve(0xac11b, 0xac127, "opera house invoke battle ultros", field.NOP())
        
        # use CA/C0FD to set the battle music override with D4 BC
        # shift the event battle space from CA/C11B to CA/C11D
        # use CA/C126 to clear the music override with D5 BC
        space = Reserve(0xac11b, 0xac11c, "set battle song override bit")
        #space.write(0xD4, 0xBC)
        space.write(field.SetEventBit(event_bit.TEMP_BATTLE_SONG_OVERRIDE))
        space = Reserve(0xac124, 0xac127, "clear battle song override bit")
        #space.write(0xD5, 0xBC)
        space.write(field.StartSong(34))
        #space.write(field.ClearEventBit(event_bit.TEMP_BATTLE_SONG_OVERRIDE))
        space.write(field.ClearEventBit(event_bit.TEMP_BATTLE_SONG_OVERRIDE))
        
        space = Reserve(0xac11d, 0xac123, "opera house invoke battle ultros", field.NOP())
        ### End of: Mod to force Grand Finale to play during this battle
