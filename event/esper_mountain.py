...
class EsperMountain(Event):
    ...
    def character_gate(self):
        ...
        #return [self.characters.RELM, self.characters.STRAGO]    
    ...
    def entrance_event_mod(self):
        ...
        if self.args.character_gating:
            src += [
                ...
                #field.BranchIfEventBitSet(event_bit.character_recruited(self.character_gate()[0]),"ESPER GATE CLEAR"),
                #field.ReturnIfEventBitClear(event_bit.character_recruited(self.character_gate()[1])),
                #"ESPER GATE CLEAR",                
                field.ClearEventBit(event_bit.ESPER_MOUNTAIN_GATED),
            ]
        src += [
    ...
    def statues_mod(self):
        ...
        #space = Reserve(0xbefc2, 0xbefc9, "esper mountain ultros statues scene", field.NOP())
        #space = Reserve(0xbefc2, 0xbefcb, "esper mountain ultros statues scene", field.NOP())
        space.write(
            field.SetEventBit(event_bit.ESPER_MOUNTAIN_GATED),
            field.Branch(space.end_address + 1), # skip nops
            #field.Branch(0xbf0c7),
        )

##        for x in range(1420):
##            space = Allocate(Bank.CB, 10, "test", field.NOP())
##            space.write(field.Return(), field.Return(), field.Return(), field.Return(),
##                        field.Return(), field.Return(), field.Return(), field.Return(),
##                        field.Return(), )
