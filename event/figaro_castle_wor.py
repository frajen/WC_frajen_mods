...
class FigaroCastleWOR(Event):
    ...
    def mod(self):
        ...
        # Mod to move airship to desert after Engine Room check
        self.airship_on_desert_mod()  
    ...
    ### Mod to have the airship follow you to the Figaro Castle location
    ### after completing the Engine Room check
    ### Note that this event takes quite some time to process vs a normal transition
    def airship_on_desert_mod(self):

        ### this moves airship when you are coming up the stairs
        ### from Fig Cas upper basement, after defeating tentacles
        src = [
            field.ReturnIfEventBitClear(event_bit.IN_WOR), # if in WoR
            field.ReturnIfEventBitClear(event_bit.DEFEATED_TENTACLES_FIGARO),
            field.SetEventBit(event_bit.TEMP_SONG_OVERRIDE),
            field.FadeLoadMap(0x01, direction.DOWN, default_music = False,
                              x = 81, y = 86, fade_in = False, airship = True),
            vehicle.SetPosition(81, 86),
            vehicle.ClearEventBit(event_bit.TEMP_SONG_OVERRIDE),
            vehicle.LoadMap(0x3d, direction.RIGHT, default_music = True, x = 3, y = 36),
            field.FadeInScreen(),
            field.Return(),
        ]
        space = Write(Bank.CA, src, "fig cas wor airship follow")
        ...
        self.maps.add_event(0x3e, new_event)

    ### End of: Mod to have the airship follow you to the Figaro Castle location
