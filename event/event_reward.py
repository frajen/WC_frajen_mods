...
def choose_tiered_reward(possible_types, characters, espers, items, event_depth, event):
    import random
    ...
    for reward_type in all_types:
        if reward_type & possible_types:
            if reward_type == RewardType.CHARACTER and characters.get_available_count():
                ...
                return (espers.get_random_tiered_esper(event_depth, event), reward_type)
            elif reward_type == RewardType.ITEM:
                item_possible = True
    ...
    # probably running out of chars and espers and need to make item rewards possible for more events
    assert(item_possible)
    return (items.get_tiered_good_random(event_depth, event), RewardType.ITEM)
...
def reward_slot_weight(slot_iterations, slot, iteration):
    ...
    ### Mod to add pathing depth to pathing placement algorithm
    import args    
    cmod = args.pathing_depth**(iteration - slot_iterations[slot])
    for s in slot_iterations:
        csum += args.pathing_depth**(iteration - s) / cmod 
    return 1 / csum
