...
class Space():
    ...
    ### Added to parse Bank space more cleanly.
    ### Use this in wc.py at the very end: Space.bank_space_available()
    def bank_space_available():
        for a in Space.heaps:
            bank_start = (a // BANK_SIZE) * BANK_SIZE
            heap = Space.heaps[Bank(bank_start)]
            print(str(a) + " | " + str(heap))
