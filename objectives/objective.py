...
class Objective:
    ...
    def _init_conditions(self, arg_conditions):
        ...
        ### Framework for adjusting randomized bosses
        WoR_bosslist = [3, 7, 8, 13, 18, 22, 23, 26, 27, 28, 29, 35, 38]

        # next, initialize conditions with 'Random' argument chosen
        for index, arg_condition in enumerate(arg_conditions):
            if arg_condition.name != "Random" and arg_condition.args[0] == 'r':
                ### section to force individual Random Boss selection to pick from a
                ### specific list
                #if arg_condition.name == "Boss":
                    #random_arg = random.choice(WoR_bosslist)
                    #WoR_bosslist.remove(random_arg)
                #else:                
